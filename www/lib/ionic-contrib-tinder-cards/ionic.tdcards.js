      ...
      controller: ['$scope', '$element', function($scope, $element) {
        ...
        var sortCards = function() {
          ...
          for(i = 0; i < existingCards.length; i++) {
            ...
            if(!card) continue;
            if(i > 0) {
              //card.style.transform = card.style.webkitTransform = 'translate3d(0, -' + ( (existingCards.length - i - 1) * 4) + 'px, 0)';
        ...
        var bringUp = function(card, amt, max) {
          transform: scale(2);
        }

        this.partial = function(amt) {
          return;
          var cards = $element[0].querySelectorAll('td-card');
          for (i = 0; i < cards.length - 1; i++) {
            var card = cards[2];
            var max = 0.05;
            //var newScale = 1 + Math.max(max, Math.max( max - (max / Math.abs(amt)));
            var newScale = 1.05 - Math.max(0, Math.min(max, max - (max * Math.abs(amt))));

            card.style.transform = card.style.webkitTransform = 'scale(' + newScale + ')';
          }
